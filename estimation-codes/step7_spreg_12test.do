//==============================================================================
//	STEP 7 : SPECIAL REGRESSIONS (2012) 
//==============================================================================
//  Instructions:
//------------------------------------------------------------------------------
...
// open logfile ... 
//------------------------------------------------------------------------------
log using "C:\ihds\step7_spreg_12test", replace
...
foreach m of local depvars { 
	...
	gen missing = 0
        ...
        local x
	local x  `m' `cvars1' `elecvar' `inst' stateid groups8x cs4x 
	...
	pe keep if inlist( sample, 1 ) // reg sample 
        ...
        // create variables ... 
        //--------------------------------------------------------------------------
        pe tabulate stateid , gen( stvars ) // state variables ...
            pe drop stvars1 // drop first dummy to prevent collinearity

        pe tabulate groups8x , gen( crvars ) // caste/religion variables ... 
            pe drop crvars1 // drop first dummy to prevent collinearity
        //--------------------------------------------------------------------------
	...
	pe eststo: sspecialreg `m' cs4x , kdens endog( `elecvar' ) bs bsreps(10) exog( `cvars1' `ovars' ) iv( `inst' ) 
...
esttab using step7_spreg_12test.tex , b(a6) se(6) wide starlevels("\sym{*}" 0.100 "\sym{**}" 0.050 "\sym{***}" 0.010) replace 
